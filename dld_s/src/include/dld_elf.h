...
#define	ELF_TYPE_EXE		2	//relocated files
...
#define	SECTION_DATA_START_ADDR	0x1000	//0x34(elf_header) + 0x20 * 3(all the program header) + 0x28 * 13(all the section header)
...
{
	u32	addr;
	s8	label[ELF_IDEN_LEN];	//labe[16]="_start"
};
...
u32	file_offset, vir_offset, vir_data_offset;

u32	elf_header_offset;
u32	elf_header_size;		//0x34
u32	program_header_offset; 		//0x34
u32	program_header_size;		//text,data,bss:: 0x20 * 3 = 0x60
u32	section_header_offset;		// 0x34 + 0x60 = 0x94 + 2
u32	section_header_size;		// 40 * 9 = 360

u32	section_content_offset;

struct entry g_entry;
